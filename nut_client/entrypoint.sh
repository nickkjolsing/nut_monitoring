...
# SHUTDOWN_GRACE → seconds to wait before the halt actually begins
grace="${SHUTDOWN_GRACE:-0}"
if [ "$grace" -gt 0 ]; then
  echo "nut-client: event '$1' -> powering off host in ${grace}s"
  sleep "$grace"
fi
...
if [ "${DRY_RUN:-false}" = "true" ]; then
  echo "DRY_RUN enabled -> shutdowns will be logged, NOT executed"
else
  echo "DRY_RUN off -> a forced shutdown (fsd message) WILL power off this host"
fi
